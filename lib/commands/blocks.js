...
function listBuildingBlocks(args, options, callback) {
  fetchUrl('foundation.zurb.com', '/building-blocks/data/building-blocks.json', function(blocksJson) {
    ...
    var blocksSorted = {};
    //
    if (options.sort || options.sorted) {
      _.each(blocks, function(properties, block) {
        var category = properties.category;
        //
        if ( blocksSorted[category] === undefined ) {
          blocksSorted[category] = [];
        }
        blocksSorted[category].push(term.str.white(block+": ")+term.str.green(properties.name))
      });
      _.each(blocksSorted, function (v, category) {
        i++;
        index = (" "+i).substr(-3);
        term.dim(index + ") ").brightMagenta(category)(":\n")
        var mapped = blocksSorted[category].sort().map(function (block) {
          term.white('   • ')(block)('\n')
        })
      });
    } else {
      _.each(blocks, function(properties, block) {
        i++;
        index = (" "+i).substr(-3);
        term.dim(index + ") ").magenta(block)(": " + properties.name + "\n");
      });
    }
