...
var blocksJSON = pkg.foundationRepos.blocks.JSON;
var blocksBase = pkg.foundationRepos.blocks.base;
...
function listBuildingBlocks(args, options, callback) {
  fetchBuildingBlocks('foundation.zurb.com', blocksJSON, function(data) {
    var i = 0;
    var index = "";
...
function installFiles(name, callback) {
  async.map(FILETYPES, function(filetype, cb) {
    ...
    var path = pjoin(blocksBase, name, filename);
...
function downloadZipFile(name, callback) {
  ...
  var path = pjoin(blocksBase, filename);
  fetchUrl('foundation.zurb.com', path, function(file) {
    if(file.length > 0) {
