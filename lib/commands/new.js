...
module.exports = function(args, options, callback, ee) {
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(options), function(answers) {
      ...
      projectFolder = directory + "/" + projectName
  ...
  function gitClone(cb) {
    ...
    exec(cmd, function(err) {
      if (err instanceof Error) {
        ...
        if(typeof(ee) !== undefined) ee.emit("cloneFailure");
        process.exit(1);
      }
      process.chdir(projectFolder);
      if(typeof(ee) !== undefined) ee.emit("cloneSuccessful");
      cb();
    });
  }
  ...
  function npmInstall(cb) {
    npm.load({ prefix: projectFolder, loglevel: 'error', loaded: false }, function(err) {
      npm.commands.install([], function(err, data) {
        ...
        if(success && typeof(ee) !== undefined) ee.emit("npmInstallSuccess");
        else if(typeof(ee) !== undefined) ee.emit("npmInstallFailure");
        cb(null, success);
      });
    });
  }
  ...
  function bowerInstall(cb) {
    ...
    .on('err', function(err) {
      if(typeof(ee) !== undefined) ee.emit("bowerInstallFailure");
      cb(null, false);
    })
    .on('end', function(data) {
      // util.copySettings(function() {
        // cb(null, true);
      // });
      if(typeof(ee) !== undefined) ee.emit("bowerInstallSuccess");
      cb(null, true);
    });
  }
