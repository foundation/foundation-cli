...
var repositories = pkg['foundationRepos']
...
var usage = chalk.underline('Usage') +
  ':\n$0 new [--directory|--project <name>] [--framework <apps|emails|sites>] [--template <basic|zurb>]';


var example = "$0 new --project <name> --framework sites --template basic\n"
example += "$0 new"

exports.describe = "Create a new Foundation project"
...
exports.builder = ( yargs ) => {
  return yargs
    ...
    .option('framework',{
      alias: 'f',
      description: 'Foundation frameworks',
      nargs: 1,
    })
    .option( 'template', {
      alias: 't',
      description: 'Foundation templates',
      nargs: 1,
      type: 'string',
    })
    .option( 'directory', {
      alias: ['d','p'],
      desc: 'Name of project root folder',
      nargs: 1,
      type: 'string'
    })
    .epilogue(
      "Run the command without flags to get an interactive setup prompt."
    )
    .implies({
      'template':'framework'
    })
    .updateStrings({'Examples:': chalk.underline('Examples')+":"})
};
...
exports.handler = (argv) => {
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(argv)).then(function(answers) {
      //      The variables we need either came from the prompts, or the console arguments
      projectName = answers.directory || argv.directory;
      framework = answers.framework || argv.framework;
      template = answers.template || argv.template || 'unspecified';
  ...
  function gitClone(cb) {
    var repo = framework === 'sites'
      ? repositories.frameworks.sites[template]
      : repositories.frameworks[framework]
    ...
    if (repositories.frameworks[framework] === undefined) {
