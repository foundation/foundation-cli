...
module.exports = function(args, options, callback, ee) {
  var projectName, projectFolder, framework, template, messages, directory, frameworkVersion;
  var tasks = [
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(options), function(answers) {
      ...
      frameworkVersion = answers.frameworkVersion || options.frameworkVersion;
  ...
  function gitClone(cb) {
    ...
    if (repo === 'sites' && frameworkVersion !== undefined) {
      repo = '-b ' + frameworkVersion + ' --single-branch --depth 1  ' + repo;
    }
