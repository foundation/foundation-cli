...
module.exports = function(args, options, callback, ee) {
    ...
    function prompt(cb) {
        if(options.framework === '') {
            say("Please choose a framework.");
        }

        if(!options.framework.match(/^(site|app|email)s?$/)) {
            say("INVALID\n".red+"You have entered an unknown framework. Choose from below.");
            ...
            {
                say("INVALID\n".red+"The template "+options.template.cyan+" is incompatible with the framework "+options.framework.cyan);
                options.template = undefined;
            }
        }
