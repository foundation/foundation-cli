...
//var say = require('../util/converse.js'); // meant for use with dialogue trees hmm
...
module.exports = function(args, options, callback, ee) {
    ...
    var dialogue = {};
    dialogue.nouns = [projectName, projectFolder, framework, template, messages, directory];
    ...
    function prompt(cb) {
        if(options.directory === '') {
            term.red.error("Please choose a project name.\n\n");
        }

        if(options.framework === '') {
            term.red.error("Please choose a framework.\n\n");
        }

        if(options.framework !== undefined && !options.framework.match(/^(site|app|email)s?$/)) {
            term.red.error("INVALID! You have entered an unknown framework. Choose from below.\n\n");
            options.framework = undefined;
            options.template = undefined;
        } else {
            if(options.template !== undefined &&
               options.template !== '' &&
               !(options.template in repositories[options.framework]))
            {
                term.red.error("INVALID! The template "+options.template+" is incompatible with the framework "+options.framework+"\n\n");
                options.template = undefined;
            }
        }
        ...
        inquirer.prompt(util.questions(options), function(answers) {
            ...
            projectName = answers.project || answers.directory || options.directory;
            ...
            messages = util.messages(dialogue);
            cb();
        });
    }
    ...
    function gitClone(cb) {
        ...
        term(util.mascot(framework, hello));
	    ...
	    term.red.alert("error!\n")("Framework ").cyan(framework)( " unknown.");
	    process.exit(1);
        }
